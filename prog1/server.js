...
mongoose.connect('mongodb://10.92.128.58:27017/blogger', { useNewUrlParser: true });
...
app.get('/blogs', (req, res) => {
	Blog.getBlogs((err, blogs) => {
		User.getUsers((err, users) => {
			res.render('blog', { blogs, users });
		});
	});
});

app.get('/blogs/edit/:_id', (req, res) => {
	Blog.getBlogById(req.params._id, (err, blog) => {
		if (err) throw err;
		User.getUsers((err, users) => {
			if (err) throw err;
			res.render('blog-edit', { blog, users });
		});
	});
});
...
app.put('/blogs/edit/:_id', [
	...
	if (!errors.isEmpty()) {
		...
		const newBlog = {
			...req.body,
			comments: { comment: req.body.comment, approved: true, user_id: req.body.comment_author },
			category: req.body.category.split(',').map(cat => ({ name: cat }))
		}
...
app.delete('/blogs/delete/:_id', (req, res) => {
	Blog.removeBlog(req.params._id, (err, blog) => {
		if (err) throw err;
		res.redirect('/blogs');
	});
});
